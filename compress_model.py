...
class MixedModel(nn.Module):
    def __init__(self, batchsize, layers, hidden_dim, ffn_dim, vocab_dim, timesteps, vocab_size):
        ...
        self.lin3 = nn.Linear(hidden_dim, vocab_size)
        ...
        self.n_group = 2
        self.sgu = True
        self.fusion_layer = nn.Linear(self.layers * self.hidden_dim, self.hidden_dim)
        self.l2 = nn.Linear(self.layers, 1)
    ...
    def forward(self, x):                           # 正常的transformer的forward，计算KQV。训练的时候只训
        ...
        if len(self.last) == 0:
            for i, layer in enumerate(self.mlp):
                if i % 2 == 0:
                    ...
                    x = layer.forward(x)
                    temp = torch.stack([x, last_x])
                    temp = temp.permute(1, 2, 3, 0)
                    temp = self.LINEAR[i//2](temp)
                    x = temp.permute(3, 0, 1, 2).squeeze(0)

                    x = layer.forward(x)
                    last_x = x
                    out.append(x.reshape(bs, -1, self.hidden_dim))
        ...
        out = self.l2(out)
        out = out.permute(3, 0, 1, 2).squeeze(0)
        final = self.lin(out)
